<template>
  <div class="flex-1 flex flex-col items-center justify-center p-6">
    <h2 class="text-2xl font-black uppercase mb-12 tracking-widest flex items-center gap-3">
      <span class="w-8 h-[2px] bg-amber-500"></span>Choose Source<span class="w-8 h-[2px] bg-amber-500"></span>
    </h2>
    <div class="w-full max-w-2xl space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        <button @click="$emit('toggleSource', 'screen')"
          ...
          <div class="text-center">
            <span class="block font-black uppercase tracking-[0.2em] text-sm mb-1" :class="selectedSources.includes('screen') ? 'text-amber-500' : 'text-slate-600'">Screen Share</span>
            <span class="block text-[9px] font-bold text-slate-500 uppercase tracking-widest opacity-60">High Quality Stream</span>
          </div>
          <div v-if="selectedSources.includes('screen')" class="absolute top-6 right-6 text-amber-500"><CheckCircle2 class="w-6 h-6 fill-amber-500/10" /></div>
        </button>
        <button @click="$emit('toggleSource', 'camera')"
          ...
          <div v-if="selectedSources.includes('camera')" class="absolute top-6 right-6 text-amber-500"><CheckCircle2 class="w-6 h-6 fill-amber-500/10" /></div>
        </button>
      </div>
      ...
      <button @click="$emit('toggleSource', 'mic')"
        ...
        <div class="flex items-center gap-3">
          <span class="text-[10px] font-black uppercase tracking-widest" :class="selectedSources.includes('mic') ? 'text-amber-500' : 'text-slate-600'">{{ selectedSources.includes('mic') ? 'ON' : 'OFF' }}</span>
          <div class="w-10 h-5 bg-slate-950 rounded-full border border-slate-800 p-1 relative transition-colors" :class="{ 'bg-amber-500/20 border-amber-500/50': selectedSources.includes('mic') }">
            <div class="w-3 h-3 rounded-full bg-slate-700 transition-all duration-300 transform" :class="selectedSources.includes('mic') ? 'translate-x-5 bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'translate-x-0'"></div>
          </div>
        </div>
      </button>
    </div>
